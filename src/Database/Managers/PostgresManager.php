...
use Proton\Database\Grammars\PostgresGrammar;
...
class PostgresManager implements DatabaseManager
{
  ...
  public function create($data) 
  {
    ...
    $query = PostegresGrammar::buildInsertQuery(array_keys($data));
  ...
  public function read($columns = '*', $filter = null)
  {
    if (self::$instance === null) {
     ...
     $query = PostegresGrammar::buildSelectQuery($columns, $filter);
     $stm = self::$instance->prepare($query);
  ...
  public function update($id, $data)
  {
    if (self::$instance === null) {
     ...
     $query = PostegresGrammar::buildUpdateQuery(array_keys($data));
     $stm = self::$instance->prepare($query);
     for ($i = 1; $i <= count($values = array_values($data)); $i++) {
       $stm = bindValue($i, $values[$i - 1]);
       if ($i = count($values)) {
  ...
  public function delete($id)
  {
    if (self::$instance === null) {
     ...
     $query = PostegresGrammar::buildDeleteQuery();
     $stm = self::$instance->prepare($query);
     $stm->bindValue(1,$id);
